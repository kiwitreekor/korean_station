...
define(spritelayout_platform_fenceA, {regular(spr_fence, xyz(0, 0, get_platform_height($1)), dxdydz(16, 1, 6), aslflags({SKIP}), registers({REGISTER_PLATFORM_A_FENCE_SKIP}))})
define(spritelayout_platform_fenceB, {regular(spr_fence, xyz(0, 16, get_platform_height($1)), dxdydz(16, 1, 6), aslflags({SKIP}), registers({REGISTER_PLATFORM_B_FENCE_SKIP}))})

define(spritelayout_track_fenceA, {regular(spr_ttd_fence | 0x8000, xyz(0, 1, 0), dxdydz(16, 1, 6), TTD, aslflags({SKIP}), registers({REGISTER_PLATFORM_A_FENCE_SKIP}))})
define(spritelayout_track_fenceB, {regular(spr_ttd_fence | 0x8000, xyz(0, 15, 0), dxdydz(16, 1, 6), TTD, aslflags({SKIP}), registers({REGISTER_PLATFORM_B_FENCE_SKIP}))})
